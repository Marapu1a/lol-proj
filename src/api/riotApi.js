...
export const getSummonerPUUID = (gameName, tagLine) => {
    return axios.get(`${pref}riot/account/v1/accounts/by-riot-id/${gameName}/${tagLine}`)
        .then(response => {
...
export const getMatchHistory = (puuid) => {
    return axios.get(`${pref}lol/match/v5/matches/by-puuid/${puuid}/ids?count=5`) //вручную количество матчей изменил, чтоб избежать ограничения RIOT
        .then(response => {
            return response.data;
        })
        .catch(error => {
...
export const getMatchDetails = (matchId) => {
    return axios.get(`${pref}lol/match/v5/matches/${matchId}`)
        .then(response => {
            return response.data;
        })
        .catch(error => {
